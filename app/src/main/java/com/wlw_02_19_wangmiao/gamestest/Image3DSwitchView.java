...
//    如果不足5个则图片轮播器无法正常显示，直接return掉。
//    如果大于等于5个，就会通过一个for循环来为每个子视图分配显示的位置，
//    而每个子视图都是一个Image3DView，
...
//    当手指按下时会记录按下时的横坐标，然后当手指滑动时会计算出滑动的距离，
//    并调用scrollBy()方法来进行滚动，当手指离开屏幕时会距离当前滑动的距离和速度来决定，
//    是滚动到下一张图片，还是滚动到上一张图片，还是滚动回原图片。
//    分别调用的方法是scrollToNext()、scrollToPrevious()和scrollBack()。
    @Override
...
//    在scrollToNext()方法中会先计算一下还需滚动的距离，
//    然后进行一下边界检查，防止当前图片的下标超出合理范围，
//    接着会调用beginScroll()方法来进行滚动。
//    在beginScroll()方法中其实就是调用了Scroller的startScroll()方法来执行滚动操作的，
//    当滚动结束后还会调用requestLayout()方法来要求重新布局，之后onLayout()方法就会重新执行，
//    每个图片的位置也就会跟着改变了。
//    至于scrollToPrevious()和scrollBack()方法的原理也是一样的，这里就不再重复分析了。
    ...
    /**
     * 其实就是遍历了一下每个Image3DView控件，
     * 然后调用它的setRotateData()方法，
     * 并把图片的下标和滚动距离传进去，
     * 这样每张图片就知道应该如何进行旋转了。
