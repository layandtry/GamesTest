...
//    在Image3DView的构造函数中初始化了一个Camera和Matrix对象，用于在后面对图片进行3D操作。
    ...
    /**
     * 然后在initImageViewBitmap()方法中初始化了一些必要的信息，
     * 比如对当前图片进行截图，以用于后续的立体操作，得到当前图片的宽度等。
    ...
    /**
     * 接下来当图片需要绘制到屏幕上的时候就会调用onDraw()方法，
     * 在onDraw()方法中会进行判断，
     * 如果当前图片可见就调用computeRotateData()方法来计算旋转时所需要的各种数据，
     * 之后再通过Camera和Matrix来执行旋转操作就可以了。
     * @param canvas
     */
    @Override
    ...
    /**
     * 有了这两样数据就可以通过computeRotateData()方法来计算旋转角度的一些数据，
     * 以及通过isImageVisible()方法来判断出当前图片是否可见了，
     * 在这里计算所有旋转所需要的数据。
     */
